...
Character/grouping is not editable here (#678 Phase 5) — see test_grouping.py
and test_api_models.py for the durable-group merge/split/patch endpoints."""
...
class TestBulkEnrich:
    ...
    def test_partial_fields_only_touches_provided(self, client, db):
        ...
        client.patch("/models/bulk/enrich", json={"ids": [a.id], "notes": "n"})
        db.refresh(a)
        assert a.notes == "n"
    ...
    def test_empty_ids_returns_400(self, client, db):
        _setup(db)
        r = client.patch("/models/bulk/enrich", json={"ids": [], "notes": "n"})
    ...
    def test_unknown_ids_are_ignored(self, client, db):
        _, a, _, _ = _setup(db)
        r = client.patch("/models/bulk/enrich", json={"ids": [a.id, 99999], "notes": "n"})
        assert r.status_code == 200
