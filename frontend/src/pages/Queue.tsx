...
import {
  QUEUED_MODELS_PARAMS,
  useQueuedModels,
  useRecentlyPrintedModels,
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative group/drag">
      <button
        ...
        className="absolute bottom-2 left-2 z-20 p-1 rounded bg-black/60 hover:bg-black/90 text-text-primary-alt2 hover:text-white cursor-grab active:cursor-grabbing touch-none opacity-0 group-hover/drag:opacity-100 transition-opacity"
      >
...
export default function Queue() {
  ...
  const queued = queuedQuery.data?.items ?? [];
  ...
  const refreshQueue = useCallback(() => {
    invalidateModelViews(queryClient, { includeVariants: false });
  }, [queryClient]);
  ...
  const onDragEnd = async (event: DragEndEvent) => {
    ...
    queryClient.setQueryData<ModelList>(
      queryKeys.models.list(QUEUED_MODELS_PARAMS),
      (prev) => prev ? { ...prev, items: next } : prev,
    );
    try {
      await api.models.reorderQueue(next.map((m) => m.id));
      queryClient.invalidateQueries({ queryKey: queryKeys.models.list(QUEUED_MODELS_PARAMS) });
    } catch {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h1 className="text-2xl font-bold text-text-primary">Print Queue</h1>
        <span className="text-sm text-text-secondary-alt ml-1">({queued.length})</span>
      </div>
      {queued.length > 1 && (
        <p className="text-xs text-text-secondary-alt mb-6">
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
          <SortableContext items={queued.map((m) => m.id)} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {queued.map((m) => (
                <SortableCard key={m.id} model={m} onMutate={refreshQueue} selected={selection.has(m.id)} onSelect={toggleSelect} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
      ...
      {printed.length > 0 && (
        <div className="mt-12">
          <div className="flex items-center gap-2 mb-4 pb-2 border-b border-border-subtle">
            ...
            <h2 className="text-lg font-semibold text-text-primary-alt">Recently Printed</h2>
            <span className="text-sm text-text-secondary-alt">({printed.length})</span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 opacity-75">
            {printed.map((m) => (
              <div key={m.id} className="relative">
                <ModelCard model={m} backTo="/queue" onMutate={refreshQueue} />
                {m.printed_at && (
