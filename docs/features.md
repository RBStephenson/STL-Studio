...
`.3mf` project bundles, etc.) — click one to download it. A `.3mf` lands here
rather than in the main parts list: it's commonly a slicer project wrapping
multiple parts rather than a single printable piece, so it's kept alongside
the model without being treated as one of its tracked STL files. Each entry
has a delete icon that removes it both from disk and from the listing (with a
confirmation first); if the file was already gone from disk (e.g. deleted
outside the app), this still clears the
stale listing.
